...
# ── Config ────────────────────────────────────────────────────────────────────
LLM_BASE_URL = os.environ.get("LLM_BASE_URL", "https://api.llm7.io/v1")
LLM_API_KEY  = os.environ.get("LLM_API_KEY",  "any")        # llm7 không cần key thật
...
class LearningPathAgent:
    ...
    def __init__(self):
        ...
        print(f"✓ LearningPathAgent sẵn sàng ({LLM_MODEL} via llm7.io)")
...
try:
    ...
    @app.get("/health")
    async def health():
        return {"status": "ok", "model": LLM_MODEL, "provider": "llm7.io"}
